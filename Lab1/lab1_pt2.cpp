...
//Checks the SEK inputs for negative values
bool validateLast(float& input)
{
    if(input < 0)
    {
...
//Checks the stride inputs for positive value
bool validateStride(float& input)
{
    cout << input << endl;
    if(input < static_cast<float>(0.01))
    {
...
//Checks the stride inputs for positive value
bool validateTaxRate(float& input)
{
    if(input < 0)
    {
...
int main()
{
    ...
    cin >> tax_percent;
    while(!validateTaxRate(tax_percent))
    {
        cout << "Enter tax percent: ";
        cin >> tax_percent;
    }
